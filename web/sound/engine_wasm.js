/**
 * https://cdn.jsdelivr.net/npm/@assemblyscript/loader/index.js
 */
...
class Engine extends AudioWorkletProcessor {
    ...
    process(inputs, outputs, parameters) {
        ...
        if (this.memoryPointer == null || this.memorySize != length) {
            if (this.memoryPointer != null) {
                this.vm.__unpin(this.memoryPointer);
                this.vm.__collect(); // GC
            }

            this.memoryPointer = this.vm.__pin(this.vm.createArray(length));
            this.memorySize = length;
        }

        this.vm.generateSamples(this.instructionPointer, this.memoryPointer);
        const mem = this.vm.__getArrayView(this.memoryPointer); // This is unsafe, if there are issues can replace with __getArray

        outputs[0][0].set(mem);

        if (outputs[0][1]) { // TODO: Support stereo?
            outputs[0][1].set(mem);
        }
